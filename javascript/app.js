(function() {
  var $, S, Splitter, autogrow, buffer, fs, gui, handlebars, jonoeditor, marked, modal, ncp, path, rangyinputs, util,
    ...
    editorAction: function(action) {
      ...
      if (action === 'bold') {
        ...
        if (S(newsel.text).endsWith("*") && S(newsel.text).startsWith("*")) {
          ...
          return $('#contentwrite textarea').setSelection(sel.start - 1, sel.end - 1);
        } else {
          $('#contentwrite textarea').setSelection(sel.start, sel.end);
          return $('#contentwrite textarea').surroundSelectedText("*", "*");
        }
