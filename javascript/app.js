...
(function() {
  var $, Dropbox, S, Splitter, autogrow, buffer, db, fs, gui, handlebars, http, jonoeditor, marked, modal, net, path, rangyinputs, util;
  ...
  window.isOpen = function(port, host, callback) {
    var conn, executed, isOpen, onClose, onOpen, timeoutId;
    isOpen = false;
    executed = false;
    onClose = function() {
      var exectued;
  ...
  window.NodeWebkitDriver = (function() {
  ...
  window.noted = {
    ...
    init: function() {
      var copyFile, copycallback, defaults, doneamount, _base, _base1,
        _this = this;
      ...
      if (fs.existsSync(window.noted.notebookdir) === false) {
        ...
        defaults.forEach(function(file) {
          if (file !== void 0) {
            return copyFile(path.join(process.cwd(), "default_notebooks", file), path.join(window.noted.notebookdir, file), copycallback);
          }
        });
      } else {
      ...
      return (_base1 = window.localStorage).cursor != null ? (_base1 = window.localStorage).cursor : _base1.cursor = "";
    },
    initDropbox: function() {
      ...
      window.isOpen((Math.round(Math.random() * 48120) + 1024).toString(), "127.0.0.1", function(isportopen, port, host) {
        ...
        return window.client.authDriver(new window.NodeWebkitDriver({
          port: port
        }));
      });
      ...
      $('#noteControls img').click(function() {
        ...
        if ($(this).attr("id") === "new" && window.noted.currentList !== "all") {
          ...
          if ($(this).attr("id") === "share") {
            ...
            return mailto = "mailto:?subject=" + encodeURI(window.noted.currentNote) + "&body=" + encodeURI(window.noted.editor.getValue());
